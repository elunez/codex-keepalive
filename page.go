...
<style>
:root{color-scheme:light dark;--canvas:#f3f7fc;--surface:#fff;--surface-2:#f7f9fc;--ink:#17233d;--muted:#71809d;--line:#dfe6f0;--line-strong:#cbd6e5;--blue:#2870ed;--blue-soft:#eaf2ff;--green:#0eaa70;--green-soft:#e8f8f1;--red:#e13f47;--red-soft:#fff0f1;--gray-soft:#edf1f6;--shadow:0 1px 2px rgba(27,44,78,.04),0 12px 34px rgba(41,72,120,.05)}
@media(prefers-color-scheme:dark){:root{--canvas:#11151c;--surface:#1a202a;--surface-2:#202733;--ink:#f2f5fa;--muted:#9ba8bc;--line:#303947;--line-strong:#465267;--blue:#76a8ff;--blue-soft:#243757;--green:#4bd09d;--green-soft:#173d33;--red:#ff7a82;--red-soft:#4b252b;--gray-soft:#2a323e;--shadow:none}}
*{box-sizing:border-box}html,body{height:100%;min-height:100%;background:var(--canvas)}body{margin:0;min-height:100vh;color:var(--ink);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","PingFang SC","Noto Sans CJK SC",sans-serif}.shell{width:min(1400px,calc(100% - 48px));min-height:100vh;margin:0 auto;padding:24px 0 42px}.topbar{display:flex;align-items:center;justify-content:space-between;gap:20px;margin-bottom:20px}.brand{display:flex;align-items:center;gap:16px}.brand-mark{display:grid;place-items:center;width:62px;height:62px;border-radius:15px;background:var(--blue);color:#fff;box-shadow:0 10px 24px color-mix(in srgb,var(--blue) 25%,transparent)}.brand-mark svg{width:34px;height:34px}.brand-copy{min-width:0}.title-row{display:flex;align-items:center;gap:12px}.title-row h1{margin:0;font-size:27px;line-height:1.15;font-weight:760;letter-spacing:-.035em}.version{padding:5px 9px;border:1px solid color-mix(in srgb,var(--blue) 25%,var(--line));border-radius:8px;background:var(--blue-soft);color:var(--muted);font:12px/1 ui-monospace,SFMono-Regular,Menlo,monospace}.subtitle{margin:7px 0 0;color:var(--muted);font-size:14px}
...
.preset-btn{display:inline-flex;align-items:center;padding:4px 10px;border:1px dashed var(--line-strong);border-radius:6px;background:var(--surface-2);color:var(--muted);font-size:11px;cursor:pointer;transition:border-color .15s,color .15s,background .15s}
.preset-btn:hover{border-color:var(--blue);color:var(--blue);background:var(--blue-soft)}
...
.btn-cancel{border:1px solid var(--line-strong);background:var(--surface-2);color:var(--ink)}
.btn-cancel:hover{background:var(--gray-soft)}
...
<main class="shell">
  <header class="topbar">
    <div class="brand">
      ...
      <div class="brand-copy"><div class="title-row"><h1>Codex 定时唤醒</h1><span class="version" id="version">v0.0.1</span></div><p class="subtitle">按计划定时唤醒 Codex，保持服务活跃</p></div>
    </div>
  ...
  <section class="schedule" aria-label="唤醒计划">
    <div class="schedule-item"><svg viewBox="0 0 16 16" fill="currentColor" aria-hidden="true"><path d="M3.5 0a.5.5 0 0 1 .5.5V1h8V.5a.5.5 0 0 1 1 0V1h.5A1.5 1.5 0 0 1 15 2.5v12a1.5 1.5 0 0 1-1.5 1.5h-11A1.5 1.5 0 0 1 1 14.5v-12A1.5 1.5 0 0 1 2.5 1H3V.5a.5.5 0 0 1 .5-.5ZM2 5v9.5a.5.5 0 0 0 .5.5h11a.5.5 0 0 0 .5-.5V5H2Zm12-1V2.5a.5.5 0 0 0-.5-.5h-11a.5.5 0 0 0-.5.5V4h12Z"/></svg><div class="schedule-text"><span>下次执行</span><strong id="next-run">—</strong></div></div>
    <div class="schedule-item"><svg viewBox="0 0 16 16" fill="currentColor" aria-hidden="true"><path d="M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8Zm7.5-6.923C5.842 1.31 4.37 2.436 3.43 4h4.07V1.077ZM2.94 5A6.97 6.97 0 0 0 2 8c0 1.09.25 2.12.694 3H7.5V5H2.94Zm.49 7c.94 1.564 2.412 2.69 4.07 2.923V12H3.43Zm5.07 2.923c1.658-.233 3.13-1.359 4.07-2.923H8.5v2.923ZM13.306 11A6.97 6.97 0 0 0 14 8c0-1.09-.25-2.12-.694-3H8.5v6h4.806Zm-.736-7C11.63 2.436 10.158 1.31 8.5 1.077V4h4.07Z"/></svg><div class="schedule-text"><span>时区</span><strong id="timezone">—</strong></div></div>
  ...
  <section class="logs-panel">
    <header class="logs-head">
      <div class="logs-heading"><div class="logs-title"><span class="title-icon" aria-hidden="true"><svg viewBox="0 0 16 16" fill="currentColor"><path d="M14 4.5V14a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V2a2 2 0 0 1 2-2h5.5L14 4.5ZM9.5 1H4a1 1 0 0 0-1 1v12a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1V4.5L9.5 1Z"/><path d="M5 7.5A.5.5 0 0 1 5.5 7h5a.5.5 0 0 1 0 1h-5a.5.5 0 0 1-.5-.5Zm0 3a.5.5 0 0 1 .5-.5h5a.5.5 0 0 1 0 1h-5a.5.5 0 0 1-.5-.5Z"/></svg></span><div><h2>执行日志</h2><p>记录每次唤醒请求的执行结果</p></div></div></div>
      <div class="filters">
...
async function request(path,options={}){if(!managementKey)throw new Error('管理中心未保存登录信息');const headers={'Authorization':'Bearer '+managementKey};if(options.body&&!options.headers)headers['Content-Type']='application/json;charset=utf-8';const response=await fetch(API+path,{...options,headers});let data={};try{data=await response.json()}catch{}if(!response.ok)throw new Error(data.error||('请求失败：HTTP '+response.status));return data}
...
async function initialize(){managementKey=savedManagementKey();if(!managementKey){$('content').innerHTML='<div class="auth-state"><strong>未保存管理密码</strong><span>请退出管理中心，重新登录并勾选“记住密码”。</span></div>';notice('未保存管理密码',true);return}try{const data=await request('/status');renderStatus(data);await loadLogs()}catch(error){$('content').innerHTML='<div class="auth-state"><strong>无法读取插件状态</strong><span>'+safe(error.message)+'</span></div>';notice(error.message,true)}}
